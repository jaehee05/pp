...
const SEED: Release[] = [
  {
    ...
    date: '2026-07-02',
    body: `## 새 기능
- 기존에 학생이 종이로 써서 제출하던 **학원&과외 일정표**를 회원정보 페이지에서 직접 입력·저장 가능.
- 요일별(월~일) × 11행 표:
  - 합공 입실시간
  - 합공 퇴실시간 (오전 활동 전)
  - **오전 일정** (08:00~12:00, 활동 내용 텍스트)
  - 합공 (재)입실시간
  - 합공 퇴실시간 (오후 활동 전)
  - **오후 일정** (12:00~19:00)
  - 합공 (재)입실시간
  ...
  - 합공 (재)입실시간
  - 마감 퇴실시간
- 시각 필드는 \`type=time\` 입력, 활동은 텍스트 입력.
- 회원정보 페이지 메모 섹션 아래 [📋 학원&과외 일정표] 접기/펼치기 카드.
- [저장] · [전체 지우기] 버튼.

## 내부
- \`Student.weeklyPlan?: Partial<Record<WeekdayKey, DayPlan>>\` 필드 추가.
- \`DayPlan\` 인터페이스 (initialEnter / morningExit / morningActivity / … / closingExit).
- 비어있는 요일은 저장 시 자동 제거해 데이터 정리.`,
    createdAt: Date.parse('2026-07-02'),
    updatedAt: Date.parse('2026-07-02'),
  },
  {
    slug: 'v1_025',
    title: 'v1.0.25 — 회원가입 시 PIN 자동 채우기 (연락처 뒷 4자리)',
    date: '2026-07-02',
    body: `## 새 기능
- **신규 회원 등록** 시 PIN 을 연락처의 뒷 4자리로 자동 세팅.
- 회원등록 페이지 (\`/ops/register\`): 연락처 입력하면 PIN 필드가 실시간으로 뒷 4자리로 동기화. 사용자가 PIN 을 직접 편집하면 이후 자동 동기화 중단 (수동 우선).
- 관리자 회원 목록의 신규 등록 모달: PIN 필드는 폼에 없지만 저장 시 연락처 뒷 4자리를 자동으로 pin 필드에 채워 저장.

## 참고
- 기존 회원 편집 시엔 자동 갱신 안 함 (수동 설정한 PIN 보호).
- 연락처가 4자리 미만이면 자동 채우기 스킵.`,
    createdAt: Date.parse('2026-07-02'),
    updatedAt: Date.parse('2026-07-02'),
  },
  {
