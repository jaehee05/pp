...
- **이용권 노출 월** 필터 — 이용권마다 1~12월 체크박스로 시즌 제한 설정.
  - 프리셋 버튼: \`전체\` / \`여름방학 (7,8월)\` / \`겨울방학 (12,1,2월)\` / \`학기 (방학 제외)\`
  - 예: "방학 이용권" → 7,8월만 체크 → 7월 진입 시 회원정보 결제 화면에 자동 노출, 9월부터 자동 숨김.
  - 예: "일반 정기권" → "학기 (방학 제외)" 프리셋 → 7,8월엔 자동 숨김 → 방학권만 보임.

## 개선
- 이용권 목록 설명 옆에 **📅 시즌 뱃지** (월 범위 표시) 추가.
- 회원정보 이용권 선택 시 현재 월 기준으로 자동 필터링.`,
